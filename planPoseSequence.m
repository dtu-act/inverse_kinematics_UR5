function fullPath = planPoseSequence(robot, env, initialPosition, targetPositions)
%fullPath = planPoseSequence(robot, env, initialPosition, targetPositions)
%Plans the robot poses for a grid of points

%% ==== CALCULATE POSES FOR GRID OF POINTS ====
maxTries = 20; % Max tries at each position
weights = [1 1 1 0.5 0.5 0.5]; % Priority to position over orientation
fullPath = [];
prevTarget = [];
currentConfig = initialPosition;

for iPos = 1:size(targetPositions,1)
    disp(['--- Pos num. ', num2str(iPos) '/' num2str(size(targetPositions,1)) ' ---'])
    ...
    segmentPath = planSafePathBetweenConfigs(robot, env, currentConfig, goalConfig);
